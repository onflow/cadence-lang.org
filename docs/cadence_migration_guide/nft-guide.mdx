...
## A Brief Explaination of `BasicNFT` and `UniversalCollection`
...
### A Note about changes that were previously not allowed in upgrades
...
### A Note for contracts and accounts about parts of the migrations that will be handled automatically
...
### Make sure your contract still implements `NonFungibleToken`
...
### Update `contract.createEmptyCollection()` to accept an NFT type as an argument
...
### Ensure that your `NFT` resource implements `NonFungibleToken.NFT`
...
### Ensure that your `Collection` implements only `NonFungibleToken.Collection`
...
### Project-Specific Events are not needed Anymore
...
### Implement ViewResolver and use Correct Metadata views
...
### Ensure that your NFT ID usage remains consistent
...
### Add `createEmptyCollection()` functions to `NFT` and `Collection`.
...
### Update `borrowNFT()` to return `&{NonFungibleToken.NFT}?` (An Optional)
...
### Remove the Private Path and Type fields from the `NFTCollectionData` Metadata View
...
Please familiarize yourself with [the new entitlements feature](./../versioned_docs/version-1.0/language/access-control#entitlements)
...
### Add `NonFungibleToken.Withdraw` and `NonFungibleToken.Owner` entitlements to your `withdraw()` methods

Now that unrestricted casting is possible in Cadence, it is necessary to use
[entitlements](./../versioned_docs/version-1.0/language/access-control#entitlements)
...
### Update all getter functions to be `view`

Cadence 1.0 introduces [view functions](./../versioned_docs/version-1.0/language/functions#view-functions)
...
### Remove any Restricted Types in your contracts and transactions
...
### Use Correct Capability Syntax and Remove any code that references private paths
...
### Use Proper Entitlements for Account Objects

`AuthAccount` objects are not referred to as `Account`
and there are now more restrictions on how accounts can be used.

See the [Cadence 1.0 Account documentation](./../versioned_docs/version-1.0/language/accounts/) for more information.
