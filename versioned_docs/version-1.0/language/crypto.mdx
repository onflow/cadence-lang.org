...
- ECDSA (`ECDSA_P256` and `ECDSA_secp256k1`):
  ...
  The domain tag prefixes the message during the signature generation or verification before the hashing step (check [`hashWithTag`](#hashing-with-a-domain-tag)).
...
hash algorithm and the input domain separation tag ((check)[#signature-verification] for details).
