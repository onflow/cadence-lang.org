...
The `@` (at) symbol before a type is used to annotate whether the type is a [resource](./resources.mdxx).
...
The [move operator `<-`](./resources.mdxx#the-move-operator--) is like the assignment operator `=`,
but must be used when the value is a [resource](./resources.mdxx).
