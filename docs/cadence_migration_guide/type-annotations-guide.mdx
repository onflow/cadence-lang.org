...
In Cadence 1.0, support for restricted types was dropped, and replaced with intersection types. 
...
So, for example, a value of type `Vault{Receiver}` should be migrated to just a `Vault` value.

## Reference Types

Reference types from contracts written in Cadence v0.42 will need to be given entitlements in order to retain the same functionality 
in Cadence 1.0. 
