...
import { nftSnippet, defiSnippet } from "@site/src/lib/homepage-snippets";

import "./index.css";

// Ported from the cadence-lang redesign (PR #306, src/routes/index.tsx).
// Differences, all mechanical:
//   - fumadocs HomeLayout            -> @theme/Layout (noFooter; page has its own)
//   - TanStack <Link to="/docs/$">   -> @docusaurus/Link to="/docs"
//   - server-fn Shiki highlighting   -> build-time plugin, read via usePluginData
//   - MorphingAscii                  -> wrapped in <BrowserOnly> (WebGL, no SSR)
//   - bg-accent/10          -> bg-accent/10 (Tailwind v3 alpha support)
//   - AI chat trigger                -> omitted; needs a server route
// The markup, copy, and Cadence snippets are otherwise unchanged.

const TITLE = "Cadence - Smart Contracts Built for the AI Era";
const DESCRIPTION =
...
function SeoTags() {
  ...
  return (
    <Head>
      ...
      <meta property="og:logo" content={`${base}/img/logo.svg`} />
...
function Home() {
  // Highlighted at build time by plugins/cadence-highlight.
  const highlightedCode = usePluginData("cadence-highlight");
  ...
  const snippet =
    activeCodeTab === "nft"
      ? highlightedCode.nftSnippet
      : highlightedCode.defiSnippet;
            ...
            <div className="max-w-7xl mx-auto w-full z-10 grid lg:grid-cols-2 gap-12 items-center">
              ...
              <div className="flex flex-col items-start text-left">
                ...
                <div className="relative w-full max-w-[calc(100vw-3rem)] sm:max-w-lg mb-2 group">
                  <div className="absolute -inset-1 bg-gradient-to-r from-accent/30 to-blue-500/30 rounded-xl blur opacity-25 group-hover:opacity-50 transition duration-1000 group-hover:duration-200" />
                ...
                <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-10">
                  ...
                  {current.hasClientSelect && (
                    <div className="relative">
                      <select
                        ...
                        className="appearance-none text-xs font-mono bg-neutral-100 dark:bg-white/5 border border-black/5 dark:border-white/10 rounded-lg pl-3 pr-7 py-1.5 text-neutral-700 dark:text-neutral-300 outline-none focus:border-accent focus:ring-1 focus:ring-accent/30 transition-all cursor-pointer hover:border-black/20 dark:hover:border-white/20"
                      >
                        {mcpClients.map((c) => (
                  ...
                  <a
                    href={current.href}
                    className="text-xs font-mono text-accent hover:underline underline-offset-4 shrink-0 flex items-center gap-1"
                  >
              ...
              <div className="relative flex items-center justify-center lg:justify-end">
                <BrowserOnly
                  fallback={<div className="w-full h-[280px] lg:h-[700px] lg:w-[700px]" />}
                >
                  {() => {
                    const {
                      MorphingAscii,
                    } = require("@site/src/components/MorphingAscii");
                    return <MorphingAscii />;
                  }}
                </BrowserOnly>
              </div>
                  ...
                  <div className="space-y-10">
                    {(activeCodeTab === "nft" ? nftPillars : defiPillars).map((p, i) => (
                      <div key={`${activeCodeTab}-${i}`} className="group animate-[fadeIn_300ms_ease-in-out]">
                        <div className="text-[10px] font-mono text-green-600 dark:text-accent mb-2 opacity-80 dark:opacity-50 group-hover:opacity-100 transition-opacity">
                          {p.label}
                        </div>
                ...
                <div className="lg:col-span-8 min-w-0 lg:sticky lg:top-40 lg:self-start h-fit">
                  <div className="border border-black/10 dark:border-white/10 bg-white dark:bg-[#0A0A0A] rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <div className="overflow-hidden transition-[height] duration-300 ease-in-out" ref={codeContainerRef} style={{ height: codeHeight }}>
                      ...
                      >
                        <div
                          className="homepage-code-light"
                          dangerouslySetInnerHTML={{ __html: snippet.light }}
                        />
                        <div
                          className="homepage-code-dark"
                          dangerouslySetInnerHTML={{ __html: snippet.dark }}
                        />
                      </div>
                    </div>
                  </div>
          ...
          <section className="relative py-32 px-6 bg-black/[0.02] dark:bg-white/[0.01] border-t border-black/5 dark:border-white/5">
            <div className="max-w-7xl mx-auto">
              <div className="text-center mb-20">
                <span className="text-green-600 dark:text-accent font-mono text-xs tracking-widest uppercase mb-4 block">
                  The Paradigm Shift
                </span>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div className="bg-white dark:bg-[#0A0A0A] border border-green-500/20 dark:border-accent/20 rounded-2xl p-10 relative overflow-hidden shadow-sm dark:shadow-none">
                  <div className="absolute top-0 right-0 p-6">
                    <Zap className="w-6 h-6 text-green-600/50 dark:text-accent/50" />
                  </div>
                  <div className="flex items-center gap-3 mb-8 text-green-600 dark:text-accent">
                  ...
                  <div className="p-6 bg-accent/5 border border-accent/20 rounded-lg font-mono text-sm text-accent/80 leading-relaxed">
                    ...
                    <br />
                    <span className="text-accent/40 mt-2 block">
                  ...
                  <section className="relative py-24 px-6 z-10 border-t border-black/5 dark:border-white/5 bg-black/5 dark:bg-black/50">
                    ...
                    <div className="max-w-5xl mx-auto w-full relative group">
                      <div className="absolute -inset-1 bg-gradient-to-r from-accent/20 via-transparent to-blue-500/20 rounded-2xl blur-xl opacity-30 group-hover:opacity-50 transition duration-1000"></div>
